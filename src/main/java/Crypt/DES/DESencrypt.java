   ...
   * then AND with 0x0001*/
        ...
        if (length == 8) {
            ...
            if (mode.equals("ECB")) {
                ...
                if(operation.equals("encrypt")) {
                    ...
                    return XORBytes(cryptText(message, key, operation), iv);
                }
            } else {
    ...
    public static void main(String[] args) {
        try {
            ...
            System.out.println("Enter mode(ECB/CBC):");
            if(sc.hasNext()) {
                mode = sc.nextLine();
            }
            String operation = "";
            ...
            if(operation.equals("encrypt")) {
                ...
                byte[] key;
                DESkeygen keygen = new DESkeygen();
                String keyString = keygen.getKey();
                key = des.hexStringToByteArray(keyString);
                byte[] message = new byte[(int) textFile.length()];
                textFileInputStream.read(message);
                byte[] cipher = des.crypt(message, key, operation);
                byte[] b64 = Base64.getEncoder().encode(cipher);
                cipherFileOutputStream.write(b64);
                ...
                key = des.hexStringToByteArray(keyString);
                byte[] b64Cipher = new byte[(int) cipherFile.length()];
                cipherFileInputStream.read(b64Cipher);
                byte[] cipher = Base64.getDecoder().decode(b64Cipher);

                byte[] message = des.crypt(cipher, key, operation);
